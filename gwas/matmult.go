...
func checkRowTimesRowDimsCP(cryptoParams *crypto.CryptoParams, N crypto.CipherMatrix, M crypto.PlainMatrix) (nRows, inner, outCtx int, ok bool) {
	nRows, inner = len(N), len(M)
	if nRows == 0 || inner == 0 {
		return 0, 0, 0, false
	}
...
func CPMultMatRowTimesRowV3(cryptoParams *crypto.CryptoParams, N crypto.CipherMatrix, M crypto.PlainMatrix, numThreads int) crypto.CipherMatrix {
	...
	nRows, inner, outCtx, ok := checkRowTimesRowDimsCP(cryptoParams, N, M)
	if !ok {
		return nil
	}
	...
	for i := 0; i < nRows; i++ {
		...
		if err := failure.get(); err != nil {
			panic(fmt.Sprintf("CPMultMatRowTimesRowV3: masking row %d of N failed: %v", i, err))
		}

		parallelBlocks(outCtx, numThreads, func(start, end int) {
			failure.record(cryptoParams.WithEvaluator(func(eval ckks.Evaluator) error {
				for c := start; c < end; c++ {
					acc, err := accumulateColumnCP(eval, cryptoParams, M, rep, c, inner)
					if err != nil {
						return err
					}
					result[i][c] = acc
...
func accumulateColumnCP(eval ckks.Evaluator, cryptoParams *crypto.CryptoParams, M crypto.PlainMatrix, rep crypto.CipherVector, c, inner int) (*ckks.Ciphertext, error) {
	var acc *ckks.Ciphertext
	for j := 0; j < inner; j++ {
